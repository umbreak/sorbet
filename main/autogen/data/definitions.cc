...
void DSLInfo::formatString(fmt::memory_buffer &out, const std::vector<core::NameRef> &klass, const core::GlobalState &gs) const {
    if (props.empty()) {
    ...
    for (auto &it : globalDSLInfo) {
        ...
        if (allAncestors.find(CHALK_ODM_MODEL) != allAncestors.end()) {
            ...
            for (const std::vector<core::NameRef> &ancst : allAncestors) {
                ...
                if (ancst != OPUS_EVENT_DEPRECATEDFRAMEWORK_ABSTRACTEVENT && ancst != OPUS_RISK_DENYLISTS_MODEL_ABSTRACT_BLACKLIST_RECORD) {
